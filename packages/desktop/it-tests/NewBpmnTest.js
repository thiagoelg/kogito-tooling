"use strict";
var __awaiter =
  (this && this.__awaiter) ||
  function (thisArg, _arguments, P, generator) {
    function adopt(value) {
      return value instanceof P
        ? value
        : new P(function (resolve) {
            resolve(value);
          });
    }
    return new (P || (P = Promise))(function (resolve, reject) {
      function fulfilled(value) {
        try {
          step(generator.next(value));
        } catch (e) {
          reject(e);
        }
      }
      function rejected(value) {
        try {
          step(generator["throw"](value));
        } catch (e) {
          reject(e);
        }
      }
      function step(result) {
        result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected);
      }
      step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
  };
var __generator =
  (this && this.__generator) ||
  function (thisArg, body) {
    var _ = {
        label: 0,
        sent: function () {
          if (t[0] & 1) throw t[1];
          return t[1];
        },
        trys: [],
        ops: [],
      },
      f,
      y,
      t,
      g;
    return (
      (g = { next: verb(0), throw: verb(1), return: verb(2) }),
      typeof Symbol === "function" &&
        (g[Symbol.iterator] = function () {
          return this;
        }),
      g
    );
    function verb(n) {
      return function (v) {
        return step([n, v]);
      };
    }
    function step(op) {
      if (f) throw new TypeError("Generator is already executing.");
      while (_)
        try {
          if (
            ((f = 1),
            y &&
              (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) &&
              !(t = t.call(y, op[1])).done)
          )
            return t;
          if (((y = 0), t)) op = [op[0] & 2, t.value];
          switch (op[0]) {
            case 0:
            case 1:
              t = op;
              break;
            case 4:
              _.label++;
              return { value: op[1], done: false };
            case 5:
              _.label++;
              y = op[1];
              op = [0];
              continue;
            case 7:
              op = _.ops.pop();
              _.trys.pop();
              continue;
            default:
              if (!((t = _.trys), (t = t.length > 0 && t[t.length - 1])) && (op[0] === 6 || op[0] === 2)) {
                _ = 0;
                continue;
              }
              if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                _.label = op[1];
                break;
              }
              if (op[0] === 6 && _.label < t[1]) {
                _.label = t[1];
                t = op;
                break;
              }
              if (t && _.label < t[2]) {
                _.label = t[2];
                _.ops.push(op);
                break;
              }
              if (t[2]) _.ops.pop();
              _.trys.pop();
              continue;
          }
          op = body.call(thisArg, _);
        } catch (e) {
          op = [6, e];
          y = 0;
        } finally {
          f = t = 0;
        }
      if (op[0] & 5) throw op[1];
      return { value: op[0] ? op[1] : void 0, done: true };
    }
  };
Object.defineProperty(exports, "__esModule", { value: true });
var Tools_1 = require("./Tools");
var app;
var client;
beforeEach(function () {
  return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          return [4, (0, Tools_1.initApp)()];
        case 1:
          app = _a.sent();
          client = app.client;
          return [2];
      }
    });
  });
});
afterEach(function () {
  return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          return [4, app.stop()];
        case 1:
          _a.sent();
          return [2];
      }
    });
  });
});
test("new BPMN from Files page", function () {
  return __awaiter(void 0, void 0, void 0, function () {
    var newBpmnButton;
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          return [4, client.$("[data-ouia-component-id='new-blank-bpmn-file-card']")];
        case 1:
          newBpmnButton = _a.sent();
          return [4, newBpmnButton.click()];
        case 2:
          _a.sent();
          return [4, testEmptyBpmn()];
        case 3:
          _a.sent();
          return [2];
      }
    });
  });
});
test("new BPMN from Learn more page", function () {
  return __awaiter(void 0, void 0, void 0, function () {
    var sidebarButton, sidebar, learnMoreNav, newBpmnButton;
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          return [4, client.$("#nav-toggle")];
        case 1:
          sidebarButton = _a.sent();
          return [4, client.$("#page-sidebar")];
        case 2:
          sidebar = _a.sent();
          return [4, sidebar.getAttribute("class")];
        case 3:
          if (!!_a.sent().includes("expanded")) return [3, 5];
          return [4, sidebarButton.click()];
        case 4:
          _a.sent();
          _a.label = 5;
        case 5:
          return [4, client.$("[data-ouia-component-id='learn-more-nav']")];
        case 6:
          learnMoreNav = _a.sent();
          return [
            4,
            client.waitUntil(
              function () {
                return __awaiter(void 0, void 0, void 0, function () {
                  return __generator(this, function (_a) {
                    switch (_a.label) {
                      case 0:
                        return [4, learnMoreNav.isDisplayed()];
                      case 1:
                        return [2, _a.sent()];
                    }
                  });
                });
              },
              { timeout: 2000 }
            ),
          ];
        case 7:
          _a.sent();
          return [4, learnMoreNav.click()];
        case 8:
          _a.sent();
          return [4, sidebar.getAttribute("class")];
        case 9:
          if (!_a.sent().includes("expanded")) return [3, 11];
          return [4, sidebarButton.click()];
        case 10:
          _a.sent();
          _a.label = 11;
        case 11:
          return [4, client.$("[data-ouia-component-id='create-bpmn-button']")];
        case 12:
          newBpmnButton = _a.sent();
          return [4, newBpmnButton.click()];
        case 13:
          _a.sent();
          return [4, testEmptyBpmn()];
        case 14:
          _a.sent();
          return [2];
      }
    });
  });
});
function testEmptyBpmn() {
  return __awaiter(this, void 0, void 0, function () {
    var iframe,
      propertiesButton,
      processName,
      _a,
      explorerDiagramButton,
      nodes,
      nodeNames,
      title,
      _b,
      saveButton,
      _c,
      closeButton,
      _d,
      headerBrand,
      _e,
      _f,
      newFileActions,
      _g;
    var _this = this;
    return __generator(this, function (_h) {
      switch (_h.label) {
        case 0:
          return [4, client.$("#kogito-iframe")];
        case 1:
          iframe = _h.sent();
          return [4, client.switchToFrame(iframe)];
        case 2:
          _h.sent();
          return [4, (0, Tools_1.waitLoading)(client)];
        case 3:
          _h.sent();
          return [4, client.$("[data-ouia-component-id='docks-item-DiagramEditorPropertiesScreen']")];
        case 4:
          propertiesButton = _h.sent();
          return [4, propertiesButton.click()];
        case 5:
          _h.sent();
          return [4, client.$("input[name$='.diagramSet.name']")];
        case 6:
          processName = _h.sent();
          _a = expect;
          return [4, processName.getValue()];
        case 7:
          _a.apply(void 0, [_h.sent()]).toEqual("unsaved file");
          return [4, client.$("[data-ouia-component-id='docks-item-ProjectDiagramExplorerScreen']")];
        case 8:
          explorerDiagramButton = _h.sent();
          return [4, explorerDiagramButton.click()];
        case 9:
          _h.sent();
          return [4, client.$$("[data-ouia-component-type='tree-item'] a")];
        case 10:
          nodes = _h.sent();
          return [
            4,
            Promise.all(
              nodes.map(function (n) {
                return __awaiter(_this, void 0, void 0, function () {
                  return __generator(this, function (_a) {
                    switch (_a.label) {
                      case 0:
                        return [4, n.getText()];
                      case 1:
                        return [2, _a.sent()];
                    }
                  });
                });
              })
            ),
          ];
        case 11:
          nodeNames = _h.sent();
          expect(nodeNames).toEqual(["unsaved file"]);
          return [4, client.switchToParentFrame()];
        case 12:
          _h.sent();
          return [4, client.$("[data-testid='toolbar-title'] > h3")];
        case 13:
          title = _h.sent();
          _b = expect;
          return [4, title.getText()];
        case 14:
          _b.apply(void 0, [_h.sent()]).toEqual("unsaved file");
          return [4, client.$("[data-ouia-component-id='save-button']")];
        case 15:
          saveButton = _h.sent();
          _c = expect;
          return [4, saveButton.isDisplayed()];
        case 16:
          _c.apply(void 0, [_h.sent()]).toEqual(true);
          return [4, client.$("[data-ouia-component-id='close-button']")];
        case 17:
          closeButton = _h.sent();
          _d = expect;
          return [4, closeButton.isDisplayed()];
        case 18:
          _d.apply(void 0, [_h.sent()]).toEqual(true);
          return [4, client.$(".pf-c-brand")];
        case 19:
          headerBrand = _h.sent();
          _e = expect;
          return [4, headerBrand.getAttribute("alt")];
        case 20:
          _e.apply(void 0, [_h.sent()]).toEqual("bpmn kogito logo");
          _f = expect;
          return [4, headerBrand.getAttribute("src")];
        case 21:
          _f.apply(void 0, [_h.sent()]).toContain("images/bpmn_kogito_logo.svg");
          return [4, closeButton.click()];
        case 22:
          _h.sent();
          return [4, client.$("[data-ouia-component-id='new-file-gallery']")];
        case 23:
          newFileActions = _h.sent();
          _g = expect;
          return [4, newFileActions.isDisplayed()];
        case 24:
          _g.apply(void 0, [_h.sent()]).toEqual(true);
          return [2];
      }
    });
  });
}
//# sourceMappingURL=NewBpmnTest.js.map
