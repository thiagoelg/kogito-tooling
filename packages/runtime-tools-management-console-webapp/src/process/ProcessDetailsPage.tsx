...
export const ProcessDetailsPage: React.FC<
  RouteComponentProps<MatchProps, StaticContext, LocationState> & OUIAProps
> = ({ ...props }) => {
  ...
  useEffect(() => {
    window.onpopstate = () => {
      history.push({ state: Object.assign({}, props.location.state) });
    };
  ...
  useEffect(() => {
    ...
    if (!isLoading && !error && !processInstance) {
      ...
      if (currentPage) {
        ...
        prevPath = tempPath.filter((item: any) => item);
      }
      history.push({
        pathname: "/NoData",
        state: {
